...
3.3.0 以降、root なしで初めて KernelSU をインストールする場合は、「ファイルを選択しパッチを当てる」で boot イメージをパッチしたあとに「純正イメージとしてバックアップ」を選択できます。この場合、バックアップイメージはマネージャーの内部ストレージに保存され、パッチ済みイメージを書き込んで起動したあと、初回起動時にそのバックアップが自動的に `/data/adb/ksu` へ移動されます。
:::
