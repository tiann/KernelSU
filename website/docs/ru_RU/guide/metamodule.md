...
- **Монтирование OverlayFS**: Традиционный подход с поддержкой слоя чтения-записи (через `mm-overlayfs`)
...
::: warning ВАЖНО
Без установленного метамодуля модули **НЕ** будут смонтированы. Свежие установки KernelSU требуют установки метамодуля (например, `mm-overlayfs`) для работы модулей.
:::
...
1. Загрузите ZIP-файл метамодуля (например, `mm-overlayfs.zip`)
2. Откройте приложение KernelSU Manager
3. Нажмите на плавающую кнопку действия (➕)
4. Выберите ZIP-файл метамодуля
5. Перезагрузите устройство

Метамодуль `mm-overlayfs` — это официальная эталонная реализация, которая обеспечивает традиционное монтирование модулей на основе overlayfs с поддержкой образов ext4.
...
Если вы разрабатываете обычные модули KernelSU, вам не нужно слишком беспокоиться о метамодулях. Ваши модули будут работать, пока у пользователей установлен совместимый метамодуль (например, `mm-overlayfs`).
...
### Реальный Пример: mm-overlayfs

Метамодуль `mm-overlayfs` является официальной эталонной реализацией. Он демонстрирует лучшие практики разработки метамодулей.

#### Архитектура

`mm-overlayfs` использует **архитектуру с двумя каталогами**:
...
Вот как `mm-overlayfs` реализует обработчик монтирования:
