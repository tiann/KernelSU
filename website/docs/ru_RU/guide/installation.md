...
Начиная с версии 3.3.0, если вы впервые устанавливаете KernelSU без root, после патча boot-образа через пункт «Выбрать файл» можно выбрать «Сохранить резервную копию как оригинальный образ». В этом случае резервная копия будет сохранена во внутреннем хранилище менеджера; после прошивки и загрузки пропатченного образа при первом открытии менеджера эта копия автоматически переместится в `/data/adb/ksu`.
:::
