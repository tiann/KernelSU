...
- `actionIcon` и `webuiIcon` — необязательные пути к изображениям, которые
  ...
  менеджере. Рекомендуется указывать относительные пути; они разрешаются от
  корня каталога модуля. Например, `actionIcon=icon/icon.png` будет
  интерпретирован как `<MODDIR>/icon/icon.png`.
