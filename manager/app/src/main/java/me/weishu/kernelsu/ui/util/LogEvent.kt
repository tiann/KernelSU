...
fun getBugreportFile(context: Context): File {
    ...
    shell.newJob().add("ORIG=$(cat /proc/sys/kernel/kptr_restrict); echo 1 > /proc/sys/kernel/kptr_restrict; cat /proc/kallsyms > ${kernelConfig.absolutePath}; echo $ORIG > /proc/sys/kernel/kptr_restrict").exec()
