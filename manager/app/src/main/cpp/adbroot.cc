...
std::string GetProperty(const std::string &key, const std::string &default_value) {
    ...
    if (key == "service.adb.root") {
        ...
        auto res = orig_fn(key, "1");
        if (res == "0") {
            static constexpr const auto kSetconSymbol = "selinux_android_setcon";
            static auto orig_setcon = (decltype(selinux_android_setcon) *) dlsym(RTLD_NEXT, kSetconSymbol);

            // If we are requested to unroot, setcurrent to adbd
            orig_setcon("u:r:adbd:s0");
        }
        return res;
    }
