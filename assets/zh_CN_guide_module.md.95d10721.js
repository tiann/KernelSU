import{_ as s,o as e,c as n,Q as a}from"./chunks/framework.ec8f7e8e.js";const h=JSON.parse('{"title":"模块开发指南","description":"","frontmatter":{},"headers":[],"relativePath":"zh_CN/guide/module.md","filePath":"zh_CN/guide/module.md"}'),l={name:"zh_CN/guide/module.md"},o=a(`<h1 id="introduction" tabindex="-1">模块开发指南 <a class="header-anchor" href="#introduction" aria-label="Permalink to &quot;模块开发指南 {#introduction}&quot;">​</a></h1><p>KernelSU 提供了一个模块机制，它可以在保持系统分区完整性的同时达到修改系统分区的效果；这种机制通常被称之为 systemless。</p><p>KernelSU 的模块运作机制与 Magisk 几乎是一样的，如果你熟悉 Magisk 模块的开发，那么开发 KernelSU 的模块大同小异，你可以跳过下面有关模块的介绍，只需要了解 <a href="./difference-with-magisk.html">KernelSU 模块与 Magisk 模块的异同</a>。</p><h2 id="busybox" tabindex="-1">Busybox <a class="header-anchor" href="#busybox" aria-label="Permalink to &quot;Busybox&quot;">​</a></h2><p>KernelSU 提供了一个功能完备的 BusyBox 二进制文件（包括完整的SELinux支持）。可执行文件位于 <code>/data/adb/ksu/bin/busybox</code>。 KernelSU 的 BusyBox 支持运行时可切换的 &quot;ASH Standalone Shell Mode&quot;。 这种独立模式意味着在运行 BusyBox 的 ash shell 时，每个命令都会直接使用 BusyBox 中内置的应用程序，而不管 PATH 设置为什么。 例如，<code>ls</code>、<code>rm</code>、<code>chmod</code> 等命令将不会使用 PATH 中设置的命令（在Android的情况下，默认情况下分别为 <code>/system/bin/ls</code>、<code>/system/bin/rm</code> 和 <code>/system/bin/chmod</code>），而是直接调用 BusyBox 内置的应用程序。 这确保了脚本始终在可预测的环境中运行，并始终具有完整的命令套件，无论它运行在哪个Android版本上。 要强制一个命令不使用BusyBox，你必须使用完整路径调用可执行文件。</p><p>在 KernelSU 上下文中运行的每个 shell 脚本都将在 BusyBox 的 ash shell 中以独立模式运行。对于第三方开发者相关的内容，包括所有启动脚本和模块安装脚本。</p><p>对于想要在 KernelSU 之外使用这个“独立模式”功能的用户，有两种启用方法:</p><ol><li>设置环境变量 <code>ASH_STANDALONE</code> 为 <code>1</code>。例如：<code>ASH_STANDALONE=1 /data/adb/ksu/bin/busybox sh &lt;script&gt;</code></li><li>使用命令行选项切换：<code>/data/adb/ksu/bin/busybox sh -o standalone &lt;script&gt;</code></li></ol><p>为了确保所有后续的 <code>sh</code> shell 都在独立模式下执行，第一种是首选方法（这也是 KernelSU 和 KernelSU 管理器内部使用的方法），因为环境变量会被继承到子进程中。</p><div class="tip custom-block"><p class="custom-block-title">与 Magisk 的差异</p><p>KernelSU 的 BusyBox 现在是直接使用 Magisk 项目编译的二进制文件，<strong>感谢 Magisk！</strong> 因此，你完全不用担心 BusyBox 脚本与在 Magisk 和 KernelSU 之间的兼容问题，因为他们是完全一样的！</p></div><h2 id="kernelsu-modules" tabindex="-1">KernelSU 模块 <a class="header-anchor" href="#kernelsu-modules" aria-label="Permalink to &quot;KernelSU 模块 {#kernelsu-modules}&quot;">​</a></h2><p>KernelSU 模块就是一个放置在 <code>/data/adb/modules</code> 内且满足如下结构的文件夹：</p><div class="language-txt vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">txt</span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#e1e4e8;">/data/adb/modules</span></span>
<span class="line"><span style="color:#e1e4e8;">├── .</span></span>
<span class="line"><span style="color:#e1e4e8;">├── .</span></span>
<span class="line"><span style="color:#e1e4e8;">|</span></span>
<span class="line"><span style="color:#e1e4e8;">├── $MODID                  &lt;--- 模块的文件夹名称与模块 ID 相同</span></span>
<span class="line"><span style="color:#e1e4e8;">│   │</span></span>
<span class="line"><span style="color:#e1e4e8;">│   │      *** 模块配置文件 ***</span></span>
<span class="line"><span style="color:#e1e4e8;">│   │</span></span>
<span class="line"><span style="color:#e1e4e8;">│   ├── module.prop         &lt;--- 此文件保存模块相关的一些配置，如模块 ID、版本等</span></span>
<span class="line"><span style="color:#e1e4e8;">│   │</span></span>
<span class="line"><span style="color:#e1e4e8;">│   │      *** 模块内容 ***</span></span>
<span class="line"><span style="color:#e1e4e8;">│   │</span></span>
<span class="line"><span style="color:#e1e4e8;">│   ├── system              &lt;--- 这个文件夹通常会被挂载到系统</span></span>
<span class="line"><span style="color:#e1e4e8;">│   │   ├── ...</span></span>
<span class="line"><span style="color:#e1e4e8;">│   │   ├── ...</span></span>
<span class="line"><span style="color:#e1e4e8;">│   │   └── ...</span></span>
<span class="line"><span style="color:#e1e4e8;">│   │</span></span>
<span class="line"><span style="color:#e1e4e8;">│   │      *** 标记文件 ***</span></span>
<span class="line"><span style="color:#e1e4e8;">│   │</span></span>
<span class="line"><span style="color:#e1e4e8;">│   ├── skip_mount          &lt;--- 如果这个文件存在，那么模块的 \`/system\` 将不会被挂载</span></span>
<span class="line"><span style="color:#e1e4e8;">│   ├── disable             &lt;--- 如果这个文件存在，那么模块会被禁用</span></span>
<span class="line"><span style="color:#e1e4e8;">│   ├── remove              &lt;--- 如果这个文件存在，下次重启的时候模块会被移除</span></span>
<span class="line"><span style="color:#e1e4e8;">│   │</span></span>
<span class="line"><span style="color:#e1e4e8;">│   │      *** 可选文件 ***</span></span>
<span class="line"><span style="color:#e1e4e8;">│   │</span></span>
<span class="line"><span style="color:#e1e4e8;">│   ├── post-fs-data.sh     &lt;--- 这个脚本将会在 post-fs-data 模式下运行</span></span>
<span class="line"><span style="color:#e1e4e8;">│   ├── post-mount.sh       &lt;--- 这个脚本将会在 post-mount 模式下运行</span></span>
<span class="line"><span style="color:#e1e4e8;">│   ├── service.sh          &lt;--- 这个脚本将会在 late_start 服务模式下运行</span></span>
<span class="line"><span style="color:#e1e4e8;">│   ├── boot-completed.sh   &lt;--- 这个脚本将会在 Android 系统启动完毕后以服务模式运行</span></span>
<span class="line"><span style="color:#e1e4e8;">|   ├── uninstall.sh        &lt;--- 这个脚本将会在模块被卸载时运行</span></span>
<span class="line"><span style="color:#e1e4e8;">│   ├── system.prop         &lt;--- 这个文件中指定的属性将会在系统启动时通过 resetprop 更改</span></span>
<span class="line"><span style="color:#e1e4e8;">│   ├── sepolicy.rule       &lt;--- 这个文件中的 SELinux 策略将会在系统启动时加载</span></span>
<span class="line"><span style="color:#e1e4e8;">│   │</span></span>
<span class="line"><span style="color:#e1e4e8;">│   │      *** 自动生成的目录，不要手动创建或者修改！ ***</span></span>
<span class="line"><span style="color:#e1e4e8;">│   │</span></span>
...
<span class="line"><span style="color:#24292e;">├── .</span></span>
<span class="line"><span style="color:#24292e;">|</span></span>
<span class="line"><span style="color:#24292e;">├── $MODID                  &lt;--- 模块的文件夹名称与模块 ID 相同</span></span>
<span class="line"><span style="color:#24292e;">│   │</span></span>
<span class="line"><span style="color:#24292e;">│   │      *** 模块配置文件 ***</span></span>
<span class="line"><span style="color:#24292e;">│   │</span></span>
<span class="line"><span style="color:#24292e;">│   ├── module.prop         &lt;--- 此文件保存模块相关的一些配置，如模块 ID、版本等</span></span>
<span class="line"><span style="color:#24292e;">│   │</span></span>
<span class="line"><span style="color:#24292e;">│   │      *** 模块内容 ***</span></span>
<span class="line"><span style="color:#24292e;">│   │</span></span>
<span class="line"><span style="color:#24292e;">│   ├── system              &lt;--- 这个文件夹通常会被挂载到系统</span></span>
<span class="line"><span style="color:#24292e;">│   │   ├── ...</span></span>
<span class="line"><span style="color:#24292e;">│   │   ├── ...</span></span>
<span class="line"><span style="color:#24292e;">│   │   └── ...</span></span>
<span class="line"><span style="color:#24292e;">│   │</span></span>
<span class="line"><span style="color:#24292e;">│   │      *** 标记文件 ***</span></span>
<span class="line"><span style="color:#24292e;">│   │</span></span>
<span class="line"><span style="color:#24292e;">│   ├── skip_mount          &lt;--- 如果这个文件存在，那么模块的 \`/system\` 将不会被挂载</span></span>
<span class="line"><span style="color:#24292e;">│   ├── disable             &lt;--- 如果这个文件存在，那么模块会被禁用</span></span>
<span class="line"><span style="color:#24292e;">│   ├── remove              &lt;--- 如果这个文件存在，下次重启的时候模块会被移除</span></span>
<span class="line"><span style="color:#24292e;">│   │</span></span>
<span class="line"><span style="color:#24292e;">│   │      *** 可选文件 ***</span></span>
<span class="line"><span style="color:#24292e;">│   │</span></span>
<span class="line"><span style="color:#24292e;">│   ├── post-fs-data.sh     &lt;--- 这个脚本将会在 post-fs-data 模式下运行</span></span>
<span class="line"><span style="color:#24292e;">│   ├── post-mount.sh       &lt;--- 这个脚本将会在 post-mount 模式下运行</span></span>
<span class="line"><span style="color:#24292e;">│   ├── service.sh          &lt;--- 这个脚本将会在 late_start 服务模式下运行</span></span>
<span class="line"><span style="color:#24292e;">│   ├── boot-completed.sh   &lt;--- 这个脚本将会在 Android 系统启动完毕后以服务模式运行</span></span>
<span class="line"><span style="color:#24292e;">|   ├── uninstall.sh        &lt;--- 这个脚本将会在模块被卸载时运行</span></span>
<span class="line"><span style="color:#24292e;">│   ├── system.prop         &lt;--- 这个文件中指定的属性将会在系统启动时通过 resetprop 更改</span></span>
<span class="line"><span style="color:#24292e;">│   ├── sepolicy.rule       &lt;--- 这个文件中的 SELinux 策略将会在系统启动时加载</span></span>
<span class="line"><span style="color:#24292e;">│   │</span></span>
<span class="line"><span style="color:#24292e;">│   │      *** 自动生成的目录，不要手动创建或者修改！ ***</span></span>
<span class="line"><span style="color:#24292e;">│   │</span></span>
...
<span class="line"><span style="color:#032F62;">&quot;</span></span></code></pre></div><p>上面的这个列表将会执行： <code>mknod $MODPATH/system/app/YouTuBe c 0 0</code> 和 <code>mknod $MODPATH/system/app/Bloatware c 0 0</code>；并且 <code>/system/app/YouTube</code> 和 <code>/system/app/Bloatware</code> 将会在模块生效后被删除。</p><p>如果你想替换掉系统的某个目录，你需要在模块目录创建一个相同路径的目录，然后为此目录设置此属性：<code>setfattr -n trusted.overlay.opaque -v y &lt;TARGET&gt;</code>；这样 overlayfs 系统会自动将系统内相应目录替换（<code>/system</code> 分区并没有被更改）。</p><p>你可以在 <code>customize.sh</code> 中声明一个名为 <code>REPLACE</code> 并且包含一系列目录的变量来执行替换操作，KernelSU 会自动为你在模块对应目录执行相关操作。例如：</p><div class="language-sh vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">sh</span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#E1E4E8;">REPLACE</span><span style="color:#F97583;">=</span><span style="color:#9ECBFF;">&quot;</span></span>
<span class="line"><span style="color:#9ECBFF;">/system/app/YouTube</span></span>
<span class="line"><span style="color:#9ECBFF;">/system/app/Bloatware</span></span>
...
<span class="line"><span style="color:#032F62;">/system/app/YouTube</span></span>
<span class="line"><span style="color:#032F62;">/system/app/Bloatware</span></span>
<span class="line"><span style="color:#032F62;">&quot;</span></span></code></pre></div><p>上面这个列表将会：自动创建目录 <code>$MODPATH/system/app/YouTube</code> 和 <code>$MODPATH//system/app/Bloatware</code>，然后执行 <code>setfattr -n trusted.overlay.opaque -v y $$MODPATH/system/app/YouTube</code> 和 <code>setfattr -n trusted.overlay.opaque -v y $$MODPATH/system/app/Bloatware</code>；并且 <code>/system/app/YouTube</code> 和 <code>/system/app/Bloatware</code> 将会在模块生效后替换为空目录。</p><div class="tip custom-block"><p class="custom-block-title">与 Magisk 的差异</p><p>KernelSU 的 systemless 机制是通过内核的 overlayfs 实现的，而 Magisk 当前则是通过 magic mount (bind mount)，二者实现方式有着巨大的差异，但最终的目标实际上是一致的：不修改物理的 <code>/system</code> 分区但实现修改 <code>/system</code> 文件。</p></div><p>如果你对 overlayfs 感兴趣，建议阅读 Linux Kernel 关于 <a href="https://docs.kernel.org/filesystems/overlayfs.html" target="_blank" rel="noreferrer">overlayfs 的文档</a></p><h3 id="system-prop" tabindex="-1">system.prop <a class="header-anchor" href="#system-prop" aria-label="Permalink to &quot;system.prop&quot;">​</a></h3><p>这个文件的格式与 <code>build.prop</code> 完全相同：每一行都是 <code>[key]=[value]</code> 的形式。</p><h3 id="sepolicy-rule" tabindex="-1">sepolicy.rule <a class="header-anchor" href="#sepolicy-rule" aria-label="Permalink to &quot;sepolicy.rule&quot;">​</a></h3><p>如果您的模块需要一些额外的 SELinux 策略补丁，请将这些规则添加到此文件中。这个文件中的每一行都将被视为一个策略语句。</p><h2 id="module-installer" tabindex="-1">模块安装包 <a class="header-anchor" href="#module-installer" aria-label="Permalink to &quot;模块安装包 {#module-installer}&quot;">​</a></h2><p>KernelSU 的模块安装包就是一个可以通过 KernelSU 管理器 APP 刷入的 zip 文件，此 zip 文件的格式如下：</p><div class="language-txt vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">txt</span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#e1e4e8;">module.zip</span></span>
<span class="line"><span style="color:#e1e4e8;">│</span></span>
