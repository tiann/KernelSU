...
int main(int argc, char *argv[])
{
    ...
    char *in_strtab =
        (char *)ko_elf.data + ko_elf.shdr[in_symtab->sh_link].sh_offset;
    char *out_strtab =
        (char *)vmlinux.data + vmlinux.shdr[out_symtab->sh_link].sh_offset;

    Elf64_Sym *in_syms =
        (Elf64_Sym *)((char *)ko_elf.data + in_symtab->sh_offset);
    ...
    for (int i = 0; i < in_sym_count; i++) {
        if (in_syms[i].st_shndx == SHN_UNDEF && in_syms[i].st_name != 0) {
            ...
            Elf64_Sym *out_sym =
                find_symbol(&vmlinux, sym_name, out_symtab, out_strtab);
