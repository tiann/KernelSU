...
static void __init free_notes_attrs(struct module_notes_attrs *notes_attrs,
			     unsigned int i)
{
	if (notes_attrs->dir) {
		while (i-- > 0)
...
void __init remove_my_kobj(void)
{
    ...
    pr_info("refcnt: %d state_in_sysfs: %d state_initialized: %d parent: 0x%lx\n", 
        mod->mkobj.kobj.kref.refcount.refs.counter,
        mod->mkobj.kobj.state_in_sysfs, 
        mod->mkobj.kobj.state_initialized, 
        (unsigned long )mod->mkobj.kobj.parent
    );
