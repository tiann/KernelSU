...
//
// This file is intentionally not part of kernelsu.ko and the main build does
// not compile it. Build it from kernel/ with "make tests" (or "ddk build
// <kmi> -- tests"); it only depends on the exported ksu_get_api() symbol and
// the public headers under api/.
//
...
static int __init ksu_api_test_init(void)
{
    ...
    ret = ksu_get_api(THIS_MODULE, KSU_KERNEL_API_VERSION, &api, sizeof(api));
    ...
    {
        struct ksu_api tmp;
        int r = ksu_get_api(THIS_MODULE, KSU_KERNEL_API_VERSION + 1, &tmp, sizeof(tmp));
        pr_info(TEST_TAG "ksu_get_api(newer) = %d (expect -EOPNOTSUPP=%d)\n", r, -EOPNOTSUPP);
        if (r >= 0)
            tmp.release(THIS_MODULE);
        r = ksu_get_api(THIS_MODULE, KSU_KERNEL_API_VERSION, &tmp, 8);
