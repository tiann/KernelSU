...
`ksu_get_api(THIS_MODULE, version, &table, sizeof(table))` takes a reference
on `kernelsu.ko` and records the caller as a loaded plugin.  A plugin must
unregister every handler and then call `release(THIS_MODULE)` from its exit
routine; a cookie that is still registered when the core finally exits is
freed by the core and must not be used afterwards.  Event and reboot
registrations require a non-NULL owner, but they do not pin the owner module:
holding a reference there would make it impossible to unload.  Unregister
returns only after SRCU readers and queued reboot task work have stopped using
the registration, so no callback code runs after it returns.  A cookie is
single-use; unregistering it twice is a bug.
...
replayable state event during registration.  The replayable state events are
`CORE_READY`, `POST_FS_DATA`, `MODULE_MOUNTED`, `BOOT_COMPLETED` and
`SELINUX_READY`; each is delivered at most once per registration.  An observer's return value is
...
syscall returns, never inside the kprobe.  The kprobe sits before the
`CAP_SYS_BOOT` check, so an unprivileged process can trigger a handler even
though its `reboot()` call fails with `EPERM`.  A handler must validate the
caller itself: the magic pair is a routing key, not a permission check.

Unloading: `ksud unload` first issues `KSU_IOCTL_PREPARE_UNLOAD`, which emits
`CORE_EXITING` to the registered handlers and returns the names of the modules
that hold the API.  ksud then removes those plugins with `delete_module()`
before removing `kernelsu.ko`; a plugin that refuses to unload aborts the
sequence.  `CORE_EXITING` is a notification only: the registry keeps
accepting calls until the core's exit routine runs, and a plugin still does
its cleanup in its own exit routine.  A retried unload delivers
`CORE_EXITING` again.

`tests/ksu_api_test.c` is a smoke test that only uses `ksu_get_api()`.  It is
not part of the default build: after building `kernelsu.ko`, run
`ddk build <kmi> -- tests` (or `make -C kernel tests` with `KDIR` set) to get
`tests/ksu_api_test.ko`, and load it with `ksud insmod tests/ksu_api_test.ko`.
It logs the registration checks at load time and prints per-event counters on
unload.
