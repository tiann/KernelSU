...
static bool replayable(enum ksu_event event)
{
    return event == KSU_EVENT_CORE_READY || event == KSU_EVENT_POST_FS_DATA || event == KSU_EVENT_MODULE_MOUNTED ||
           event == KSU_EVENT_BOOT_COMPLETED || event == KSU_EVENT_SELINUX_READY;
}
...
int ksu_unregister_event_handler(void *cookie)
{
    ...
    /* The current vector is not covered by a prior replacement.  Retire it
     * through SRCU exactly once; never free it directly after synchronize_srcu
     * because a queued call_srcu callback may still own an older vector. */
    call_srcu(&event_registry.srcu, &old->rcu, free_event_vec_rcu);
...
/* Userspace is about to unload the core: tell the plugins while they can
 * still unregister from ordinary context.  The registry keeps running so an
 * aborted unload does not strand anybody; kernelsu_exit() stops it later. */
void ksu_event_notify_core_exiting(void)
{
    struct ksu_state_event state;

    state_context(KSU_EVENT_CORE_EXITING, &state);
    ksu_event_emit(KSU_EVENT_CORE_EXITING, &state, 0);
}
